 ...
 * @author HJOW
 */
class TableEditor extends ScalaDialogModule 
{
  /*
   모듈에 대한 기본 설정들을 지정하는 곳입니다.
   주의 : 모듈 ID는 initializeComponents() 메소드 내에서 지정합니다.
   */
  var moduleName  : String  = trans("Table Editor"); // 모듈 이름을 지정합니다.
  ...
  var tableListModel : ListModel = null;
  var tableList : UIList = null;
  ...
  override def initializeComponents() =
  {
    ...
    setModuleId(7238957827598235923L);
    ...
    var rightPanel : JPanel = new JPanel();
    var leftPanel : JPanel = new JPanel();
    
    rightPanel.setLayout(new BorderLayout());
    leftPanel.setLayout(new BorderLayout());
    
    tableListModel = new DefaultListModel();
    tableList = new UIList(tableListModel);
    tableList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
    addListener(tableList);
