 ...
 *
 */
public class Communicator implements Runnable, JScriptObject
{
	private static final long serialVersionUID = -468768845135420864L;
	protected ReceiveHandler eventObject;
	protected ServerSocket serverSocket;
	protected List<Socket> connected = new Vector<Socket>();
	protected List<CommunicationUnit> units = new Vector<CommunicationUnit>();
	 ...
	 */
	public void init(int port, ReceiveHandler eventHandler) throws IOException
	{
	...
	public void close()
	{
		...
		for(CommunicationUnit units : units)
		{
			units.noMoreUse();
		}
		connected.clear();
	 ...
	 * @return 클라이언트 유닛 객체
	 */
	public CommunicationUnit getClient(int idx)
	{
		...
		while(threadSwitch)
		{
			try
			{
				if(serverSocket != null)
				{
					...
					CommunicationUnit sub = new CommunicationUnit(connection);
	 ...
	 *
	 */
	class CommunicationUnit extends Thread implements JScriptObject
	{
		...
		protected ReceiveHandler independentEvent;
		public CommunicationUnit(Socket socket) throws IOException
		{
			this.socket = socket;
		 ...
		 */
		public void setEvent(ReceiveHandler handler)
		{
